...
public class MensagemChatFacade : IMensagemChatFacade
{
    ...
    public Task<IEnumerable<MensagemChatDto>> GetByProjetoId(Guid projetoId, int take = 50) =>
        _mensagemService.GetByProjetoId(projetoId, take);

    public Task<MensagemChatDto> EnviarMensagem(Guid projetoId, EnviarMensagemCommand command) =>
        _mensagemService.EnviarMensagem(projetoId, command);
    ...
    public Task<MensagemChatDto> EnviarMensagem(Guid projetoId, Guid remetenteId, string remetenteNome, string remetentePerfil, string conteudo) =>
        _mensagemService.EnviarMensagem(projetoId, remetenteId, remetenteNome, remetentePerfil, conteudo);

    public Task MarcarComoLidas(Guid projetoId, Guid usuarioId) =>
        _mensagemService.MarcarComoLidas(projetoId, usuarioId);
}
